...
class CustomerOrder extends DataObject
{
  ...
  {
    $conn = parent::connect();
    $sql = 'SELECT '. TBL_CUSTOMER_ORDER .'.*, ' .TBL_USERS_ACCOUNT. '.username FROM ' . TBL_CUSTOMER_ORDER . ' LEFT JOIN '.TBL_USERS_ACCOUNT.' ON ' .TBL_CUSTOMER_ORDER. '.customer_id = '.TBL_USERS_ACCOUNT.'.id WHERE order_status = :order_status';
    try {
      $st = $conn->prepare($sql);
